...
interface FittedTextProps {
  wordPath: string; // SVG path in arbitrary coords
...
function getPathBBox(path: string) {
  ...
  for (const cmd of commands) {
    // Check all relevant coordinates
    (["x", "x1", "x2"] as const).forEach((key) => {
      if (cmd[key] !== undefined) {
        minX = Math.min(minX, cmd[key]!);
        maxX = Math.max(maxX, cmd[key]!);
      }
    });
    (["y", "y1", "y2"] as const).forEach((key) => {
      if (cmd[key] !== undefined) {
...
// Normalize path coords to fit boxWidth x boxHeight (default 100x20)
function normalizePath(path: string): { path: string; width: number; height: number } {
    const bbox = getPathBBox(path);
  
    if (bbox.width === 0 || bbox.height === 0) {
      return { path, width: bbox.width, height: bbox.height };
    }
    ...
    const normalizedCommands = commands.map((cmd) => {
      ...
      (["x", "x1", "x2"] as const).forEach((key) => {
        if (clone[key] !== undefined) {
          clone[key] = clone[key]! - bbox.minX;
        }
      });
      (["y", "y1", "y2"] as const).forEach((key) => {
        if (clone[key] !== undefined) {
          clone[key] = clone[key]! - bbox.minY;
        }
      });
    ...
    const normalizedPath = SVGPathData.encode(normalizedCommands);
  
    return { path: normalizedPath, width: bbox.width, height: bbox.height };
  }
...
function warpPath(
  ...
  const warped = commands.map((cmd) => {
    ...
    if ("x" in clone && "y" in clone && clone.x !== undefined && clone.y !== undefined) {
      const nx = clone.x / width;
      const ny = clone.y / height;
      const warpedPoint = bilinearWarp(nx, ny, quad);
      clone.x = warpedPoint.x;
      clone.y = warpedPoint.y;
    }
    ...
    if (
      ...
      const warpedPoint = bilinearWarp(nx, ny, quad);
      clone.x1 = warpedPoint.x;
      clone.y1 = warpedPoint.y;
    }
    ...
    if (
      ...
      const warpedPoint = bilinearWarp(nx, ny, quad);
      clone.x2 = warpedPoint.x;
      clone.y2 = warpedPoint.y;
    }
...
const FittedText: React.FC<FittedTextProps> = ({ wordPath, polygon }) => {
    // Normalize just by shifting to 0,0, no scaling
    const { path: normalizedPath, width, height } = normalizePath(wordPath);
  
    // Warp using the exact bbox width and height
    const warpedPath = warpPath(normalizedPath, polygon, width, height);
...
export default function App() {
  return <FittedText wordPath={dummyPath} polygon={examplePolygon} />;
}
